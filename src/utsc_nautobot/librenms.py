from utsc.core import debug_cache, shell
...
def get_data():
    ...
    for d in devices:
        ...
        try: d['ip_addresses'] = ld.get_device_ip_addresses(d['device_id']).json()['addresses']
        except KeyError: pass
        try: d['links'] = ls.get_links(d['device_id']).json()['links']
        except KeyError: pass
        try: d['vlans'] = ls.get_vlans(d['device_id']).json()['vlans']
        except KeyError: pass
