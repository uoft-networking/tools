...
class ArubaBlacklistView(APIView):
    ...
    def __init__(self, **kwargs) -> None:
        ...
        conf = settings.PLUGINS_CONFIG["utsc_nautobot"]["aruba"]
    ...
    @extend_schema(
        ...
        responses={
            200: inline_serializer(name='aruba_stm_blacklist_get', fields={
                "STA": f.CharField(help_text='The mac address which has been blocked (ex: "d2:9a:3d:db:a6:e8")'),
                "block-time(sec)": f.CharField(help_text='How many seconds ago the mac address was added to the block list'),
                "reason": f.CharField(help_text='Why the mac address was blocked'),
                "remaining time(sec)": f.CharField(help_text='How many seconds until this mac address is removed from the block list')
            }, many=True)
        },
        examples=[
            OpenApiExample('ex1', {
    ...
    @extend_schema(
        ...
        responses={
            202: inline_serializer(name='aruba_stm_blacklist_remove', fields={
                "detail": f.CharField(help_text='A message confirming the results of the operation'),
            }),
        },
        examples=[
            OpenApiExample('ex1', {"detail": "mac address '10:02:b5:27:bb:0e' has been removed from the aruba stm blocklist"})
        ]
    )
