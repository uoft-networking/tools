...
def mock_sync_data(mocker: MockerFixture):
    ...
    def bluecat_load(self, datasets):
        target_data_file = fixtures / f"bluecat_{'-'.join(sorted(list(datasets)))}_raw.pkl"
    ...
    mocker.patch("uoft.scripts._sync.BluecatTarget.load_data_raw", new=bluecat_load)

    nautobot_orig = _sync.NautobotTarget.load_data_raw

    def nautobot_load(self, datasets):
        target_data_file = fixtures / f"nautobot_{'-'.join(sorted(list(datasets)))}_raw.pkl"
    ...
    mocker.patch("uoft.scripts._sync.NautobotTarget.load_data_raw", new=nautobot_load)

    mocker.patch("uoft.scripts._sync.NautobotTarget.delete_one", return_value=None)
...
def test_sync_data(mock_sync_data, mocker):
    ...
    sm.synchronize()
    sm.commit()
    # with mocker.patch("uoft.scripts._sync.NautobotTarget.api"):
    #     sm.commit()
