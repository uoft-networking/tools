...
if TYPE_CHECKING:  # MYPY
    ...
    from uoft.core.yaml.comments import (
        CommentedKeySeq,
        CommentedMap,
        CommentedOrderedMap,
        CommentedSeq,
    ...
    from uoft.core.yaml.main import Loader
    from uoft.core.yaml.nodes import MappingNode, ScalarNode, SequenceNode
    from uoft.core.yaml.scalarfloat import ScalarFloat
    from uoft.core.yaml.scalarstring import LiteralScalarString
...
class Constructor:
    ...
    def flatten_mapping(self, node: MappingNode) -> List[Union[Tuple[int, CommentedMap], Any]]:
        """
            ...
            if key_node.tag == "tag:yaml.org,2002:merge":
                ...
                if isinstance(value_node, MappingNode):
                    ...
                    raise ConstructorError(
                        ...
                        node.start_mark,
                        _F(
                            "expected a mapping or list of mappings for merging, but found {value_node_id!s}",
    ...
    def construct_document(self, node: Union[MappingNode, SequenceNode, ScalarNode]) -> Any:
    ...
    def construct_object(self, node: Union[MappingNode, SequenceNode, ScalarNode], deep: bool = False) -> Any:
    ...
    def construct_sequence(self, node: SequenceNode, deep: bool = False) -> List[Union[str, CommentedMap]]:
        """deep is True when creating an object/mapping recursively,
        in that case want the underlying elements available during construction
        """
    ...
    def construct_mapping(self, node: MappingNode, maptyp: CommentedMap, deep: bool = False) -> None:
        ...
        if key in mapping:
            if not self.allow_duplicate_keys:
                ...
                args = [
                    "while constructing a mapping",
                    node.start_mark,
                    'found duplicate key "{}" with value "{}" (original value: "{}")'.format(key, value, mk),
                    key_node.start_mark,
                    """
                    To suppress this check see:
                        http://yaml.readthedocs.io/en/latest/api.html#duplicate-keys
    ...
    def check_set_key(self, node: MappingNode, key_node: ScalarNode, setting: CommentedSet, key: str) -> None:
        if key in setting:
            if not self.allow_duplicate_keys:
                args = [
    ...
    def construct_rt_sequence(self, node: SequenceNode, seqtyp: CommentedSeq, deep: bool = False) -> List[Any]:
        if not isinstance(node, SequenceNode):
            raise ConstructorError(
                None,
                None,
    ...
    def construct_yaml_null(self, node: ScalarNode) -> None:
        self.construct_scalar(node)  # check the validity of node, raise exception if needed
        return None
    ...
    def construct_setting(self, node: MappingNode, typ: CommentedSet, deep: bool = False) -> None:
        if not isinstance(node, MappingNode):
            raise ConstructorError(
                None,
                None,
