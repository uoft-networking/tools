...
from uoft.core.yaml.util import LazyEval
...
class Resolver:
    ...
    def add_version_implicit_resolver(
        self, version: Tuple[int, int], tag: str, regexp: LazyEval, first: List[str | None]
    ) -> None:
        if first is None:
            first = [None]
        ...
        for element in path:
            if isinstance(element, (list, tuple)):
                if len(element) == 2:
                    ...
                    raise ResolverError(_F("Invalid path element: {element!s}", element=element))
            else:
                node_check = None
                index_check = element
            if node_check is str:
                ...
                raise ResolverError(_F("Invalid node checker: {node_check!s}", node_check=node_check))
    ...
    def descend_resolver(
        self,
        current_node: Optional[Union[MappingNode, SequenceNode]],
        current_index: Optional[Union[ScalarNode, SequenceNode, int]],
    ) -> None:
    ...
    def resolve(
        self,
        kind: Union[Type[MappingNode], Type[ScalarNode], Type[SequenceNode]],
        value: Any,
        implicit: Union[bool, Tuple[bool, bool]],
    ) -> str:
