...
from uoft.core.yaml.error import StringMark
...
from uoft.core.yaml.tokens import CommentToken, Token
...
class Parser:
    ...
    def parse_document_content(self) -> Union[MappingStartEvent, SequenceStartEvent, ScalarEvent]:
        if self.scanner.check_token(DirectiveToken, DocumentStartToken, DocumentEndToken, StreamEndToken):
    ...
    def parse_block_node_or_indentless_sequence(
        self,
    ) -> Union[AliasEvent, MappingStartEvent, SequenceStartEvent, ScalarEvent]:
    ...
    def parse_node(
        self, block: bool = False, indentless_sequence: bool = False
    ) -> Union[AliasEvent, MappingStartEvent, SequenceStartEvent, ScalarEvent]:
    ...
    def parse_block_sequence_entry(
        self,
    ) -> Union[MappingStartEvent, SequenceEndEvent, ScalarEvent, AliasEvent, SequenceStartEvent]:
    ...
    def parse_indentless_sequence_entry(
        self,
    ) -> Union[SequenceEndEvent, MappingStartEvent, SequenceStartEvent, ScalarEvent]:
        if self.scanner.check_token(BlockEntryToken):
            token = self.scanner.get_token()
            self.move_token_comment(token)
            if not self.scanner.check_token(BlockEntryToken, KeyToken, ValueToken, BlockEndToken):
        ...
        if not self.scanner.check_token(FlowMappingEndToken):
            ...
            if self.scanner.check_token(KeyToken):
                ...
                if not self.scanner.check_token(ValueToken, FlowEntryToken, FlowMappingEndToken):
                    ...
                    return self.process_empty_scalar(token.end_mark)
            elif self.resolver.processing_version > (1, 1) and self.scanner.check_token(ValueToken):
                self.state = self.parse_flow_mapping_value
    ...
    def process_empty_scalar(
        self, mark: StringMark, comment: Optional[Union[List[None], List[Optional[CommentToken]]]] = None
    ) -> ScalarEvent:
