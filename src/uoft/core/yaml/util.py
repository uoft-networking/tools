...
def create_timestamp(
    ...
    if tz_sign:
        ...
        delta = datetime.timedelta(hours=tz_hour, minutes=tz_minute, seconds=1 if frac > MAX_FRAC else 0)
        if tz_sign == "-":
            delta = -delta
    elif frac > MAX_FRAC:
...
def load_yaml_guess_indent(stream: str, **kw) -> Union[Tuple[CommentedMap, int, None], Tuple[CommentedMap, int, int]]:
