...
class Node:
    ...
    def __init__(
        ...
        start_mark: Optional[StringMark],
        end_mark: Optional[StringMark],
    ...
    def dump(self, indent=0):
        if isinstance(self.value, str):
            sys.stdout.write(
                "{}{}(tag={!r}, value={!r})\n".format("  " * indent, self.__class__.__name__, self.tag, self.value)
            )
            if self.comment:
...
class CollectionNode(Node):
    ...
    def __init__(
        ...
        comment: Optional[
            Union[List[Optional[List[CommentToken]]], List[CommentToken], List[Optional[CommentToken]]]
        ] = None,
        anchor: Optional[Union[str, Anchor]] = None,
    ) -> None:
