...
from uoft.core.yaml.io import Reader
from uoft.core.yaml.tokens import AliasToken, AnchorToken, DirectiveToken, ScalarToken, TagToken, Token
...
class Scanner:
    ...
    def fetch_flow_collection_start(
        self, TokenClass: Union[Type[FlowSequenceStartToken], Type[FlowMappingStartToken]], to_push: str
    ) -> None:
    ...
    def fetch_flow_collection_end(
        self, TokenClass: Union[Type[FlowMappingEndToken], Type[FlowSequenceEndToken]]
    ) -> None:
    ...
    def scan_block_scalar_indicators(
        self, start_mark: StringMark
    ) -> Union[Tuple[None, int], Tuple[bool, None], Tuple[None, None]]:
        # See the specification for details.
        srp = self.reader.peek
        chomping = None
        increment = None
    ...
    def scan_block_scalar_breaks(
        self, indent: int
    ) -> Union[Tuple[List[str], StringMark], Tuple[List[Any], StringMark]]:
        # See the specification for details.
        chunks = []
        srp = self.reader.peek
        srf = self.reader.forward
                    ...
                    for k in range(length):
                        if srp(k) not in "0123456789ABCDEFabcdef":
                            raise ScannerError(
                                ...
                                start_mark,
                                _F(
                                    "expected escape sequence of {length:d} hexdecimal numbers, but found {srp_call!r}",
                                    length=length,
                                    srp_call=srp(k),
                                ),
                                self.reader.get_mark(),
    ...
    def scan_uri_escapes(self, name: str, start_mark: StringMark) -> str:
            ...
            for k in range(2):
                if srp(k) not in "0123456789ABCDEFabcdef":
                    raise ScannerError(
                        ...
                        start_mark,
                        _F(
                            "expected URI escape sequence of 2 hexdecimal numbers, but found {srp_call!r}",
                            srp_call=srp(k),
                        ),
                        self.reader.get_mark(),
                    )
        ...
        try:
            ...
            raise ScannerError(_F("while scanning an {name!s}", name=name), start_mark, str(exc), mark)
        return value
