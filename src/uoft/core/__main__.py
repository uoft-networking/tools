...
def callback(
    ...
    """
    Command-line namespace for all uoft command-line utilities. makes each uoft.* command available as a subcommand.
    """
...
# [[[cog
# tasks._coghelpers.all_projects_as_python_list()
# ]]]
ALL_PROJECTS = [
    "uoft.aruba",
    "uoft.bluecat",
...
def _add_subcommands() -> tuple[set[str], set[str]]:
    """
    Dynamically add subcommands from uoft.* packages, and add virtual subcommands for any uoft-* executables
    ...
    for p in ALL_PROJECTS:
        try:
            # look for a python package named uoft.<p> in the local virtualenv and a cli module in it
            # try to add a subcommand for it
            subapp = __import__(f"uoft.{p}.cli").cli.app
...
if __name__ == "__main__":
    ...
    mod_name = p.get_string(
        "module name",
        'Enter the full dotted name of the module you\'d like to debug. Ex: "uoft.core.nested_data"',
        # validator=v,
