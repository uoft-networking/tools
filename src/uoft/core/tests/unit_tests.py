...
def test_version():
    assert isinstance(uoft.core.__version__, str)
...
def test_file(tmp_path: Path):
    ...
    assert uoft.core.File.is_creatable(creatable)
    assert uoft.core.File.state(creatable) == uoft.core.File.creatable

    assert uoft.core.File.is_readable(readable)
    assert uoft.core.File.state(readable) == uoft.core.File.readable

    assert uoft.core.File.is_writable(writable)
    assert uoft.core.File.state(writable) == uoft.core.File.writable

    assert uoft.core.File.state(unusable) == uoft.core.File.unusable
...
class UtilsTests:
    def test_config_files(self, mock_util: "MockedUtil", caplog: "LogCaptureFixture"):
        ...
        expected_output = [
            ...
            (
                mock_util.mock_folders.root / "home/user/.config/uoft-tools/shared.yaml",
                uoft.core.File.creatable,
            ),
            (
                mock_util.mock_folders.root / "home/user/.config/uoft-tools/shared.json",
                uoft.core.File.creatable,
            ),
            (
                mock_util.mock_folders.root / "home/user/.config/uoft-tools/shared.toml",
                uoft.core.File.creatable,
            ),
            (
                mock_util.mock_folders.root / "home/user/.config/uoft-tools/example_app.ini",
                uoft.core.File.creatable,
            ),
            (
                mock_util.mock_folders.root / "home/user/.config/uoft-tools/example_app.yaml",
                uoft.core.File.creatable,
            ),
            (
                mock_util.mock_folders.root / "home/user/.config/uoft-tools/example_app.json",
                uoft.core.File.creatable,
            ),
            (
                mock_util.mock_folders.root / "home/user/.config/uoft-tools/example_app.toml",
                uoft.core.File.creatable,
            ),
        ]
    ...
    def test_config_files_env_vars(
        ...
        res = mock_util.config.files
        expected_output = [
            (mock_util.mock_folders.root / "etc/alternate/shared.ini", uoft.core.File.creatable),
            (mock_util.mock_folders.root / "etc/alternate/shared.yaml", uoft.core.File.creatable),
            (mock_util.mock_folders.root / "etc/alternate/shared.json", uoft.core.File.creatable),
            (mock_util.mock_folders.root / "etc/alternate/shared.toml", uoft.core.File.creatable),
            (
            ...
            (mock_util.mock_folders.root / "home/alternate/shared.ini", uoft.core.File.creatable),
            (
        ...
        assert f"Using {mock_util.mock_folders.site_config_env.dir} as site config directory" in caplog.text
        assert f"Using {mock_util.mock_folders.user_config_env.dir} as user config directory" in caplog.text
    ...
    def test_config_files_states(self, mock_util: "MockedUtil"):
        ...
        expected_output = [
            ...
            (
                mock_util.mock_folders.root / "home/user/.config/uoft-tools/example_app.json",
                uoft.core.File.readable,
            ),
            (
                mock_util.mock_folders.root / "home/user/.config/uoft-tools/example_app.toml",
                uoft.core.File.writable,
            ),
        ]
        for expected, actual in zip(expected_output, res):
            expected_path, expected_state = expected
    ...
    def test_get_config_file(self, mock_util: "MockedUtil"):
        ...
        with pytest.raises(uoft.core.UofTCoreError) as exc_info:
    ...
    def test_get_config_key(
        ...
        with pytest.raises(uoft.core.UofTCoreError):
