...
class TestYAML:
    ...
    @pytest.mark.skipif(
        ...
        reason="uoft.core.yaml not available",
    )
    ...
    def test_CommentedSet(self):
        from uoft.core.yaml.constructor import CommentedSet
    ...
    def test_load_all_perserve_quotes(self):
        ...
        for x in yaml.load_all(s):
            data.append(x)
        buf = uoft.core.yaml.compat.StringIO()
        yaml.dump_all(data, buf)
        out = buf.getvalue()
        print(type(data[0]["a"]), data[0]["a"])
        # out = uoft.core.yaml.round_trip_dump_all(data)
        print(out)
        assert out == s
