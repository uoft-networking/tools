...
def _convert_structure(loader):
    if loader.check_event(uoft.core.yaml.ScalarEvent):
        event = loader.get_event()
        if event.tag or event.anchor or event.value:
            return True
        else:
            return None
    elif loader.check_event(uoft.core.yaml.SequenceStartEvent):
        loader.get_event()
        sequence = []
        while not loader.check_event(uoft.core.yaml.SequenceEndEvent):
            sequence.append(_convert_structure(loader))
        loader.get_event()
        return sequence
    elif loader.check_event(uoft.core.yaml.MappingStartEvent):
        loader.get_event()
        mapping = []
        while not loader.check_event(uoft.core.yaml.MappingEndEvent):
            key = _convert_structure(loader)
            value = _convert_structure(loader)
            mapping.append((key, value))
        loader.get_event()
        return mapping
    elif loader.check_event(uoft.core.yaml.AliasEvent):
        loader.get_event()
        return "*"
    else:
        loader.get_event()
...
def test_structure(data_filename, structure_filename, verbose=False):
    ...
    try:
        with open(data_filename, "rb") as fp:
            loader = uoft.core.yaml.Loader(fp)
        while loader.check_event():
            if loader.check_event(
                uoft.core.yaml.StreamStartEvent,
                uoft.core.yaml.StreamEndEvent,
                uoft.core.yaml.DocumentStartEvent,
                uoft.core.yaml.DocumentEndEvent,
            ):
                loader.get_event()
                continue
...
def _compare_events(events1, events2, full=False):
    ...
    for event1, event2 in zip(events1, events2):
        ...
        if isinstance(event1, uoft.core.yaml.AliasEvent) and full:
            assert event1.anchor == event2.anchor, (event1, event2)
        if isinstance(event1, (uoft.core.yaml.ScalarEvent, uoft.core.yaml.CollectionStartEvent)):
            if (event1.tag not in [None, "!"] and event2.tag not in [None, "!"]) or full:
                assert event1.tag == event2.tag, (event1, event2)
        if isinstance(event1, uoft.core.yaml.ScalarEvent):
...
def test_parser(data_filename, canonical_filename, verbose=False):
    ...
    try:
        with open(data_filename, "rb") as fp0:
            events1 = list(uoft.core.yaml.parse(fp0))
        with open(canonical_filename, "rb") as fp0:
            events2 = list(uoft.core.yaml.canonical_parse(fp0))
...
def _compare_nodes(node1, node2):
    ...
    if isinstance(node1, uoft.core.yaml.ScalarNode):
...
def test_composer(data_filename, canonical_filename, verbose=False):
    ...
    try:
        with open(data_filename, "rb") as fp0:
            nodes1 = list(uoft.core.yaml.compose_all(fp0))
        with open(canonical_filename, "rb") as fp0:
            nodes2 = list(uoft.core.yaml.canonical_compose_all(fp0))
...
def _make_loader():
    ...
    class MyLoader(uoft.core.yaml.Loader):
        def construct_sequence(self, node):
            return tuple(uoft.core.yaml.Loader.construct_sequence(self, node))
...
def _make_canonical_loader():
    ...
    class MyCanonicalLoader(uoft.core.yaml.CanonicalLoader):
        def construct_sequence(self, node):
            return tuple(uoft.core.yaml.CanonicalLoader.construct_sequence(self, node))
