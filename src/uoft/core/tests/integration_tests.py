...
import uoft.core
import uoft.core.prompt
from uoft.core.types import SecretStr, DirectoryPath, FilePath

if TYPE_CHECKING:
    from _pytest.monkeypatch import MonkeyPatch  # pyright: ignore[reportPrivateImportUsage]


class _Settings(uoft.core.BaseSettings):
    a_value: str = "a_value"
    secret: SecretStr

    class Config(uoft.core.BaseSettings.Config):
        app_name = "test"
...
class PromptTests:
    ...
    def test_mocked_prompt(self, monkeypatch: "MonkeyPatch", tmp_path: Path):
        ...
        monkeypatch.setattr("uoft.core.prompt.PromptSession", mocksession)

        p = uoft.core.prompt.Prompt(uoft.core.Util("prompt_test").history_cache)
