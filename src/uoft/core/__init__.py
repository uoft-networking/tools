...
def debug_cache(func: F) -> F:
    ...
    def wrapped_func(*args, **kw):
        key = (args, frozenset(kw.items())) if kw else args
        if key not in cache:
            logger.trace(f"caching output of function `{fname}` with arguments {args} and {kw}")
            cache[key] = func(*args, **kw)
...
def shell(cmd: str, input_: str | bytes | None = None, cwd: str | Path | None = None) -> str:
    ...
    return run(cmd, shell=True, capture_output=True, check=True, text=True, input=input_, cwd=cwd).stdout.strip()
...
def create_or_update_config_file(file: Path, obj: dict[str, Any], write_as: Optional[DataFileFormats] = None):
    if file.exists():
...
class PassPath(type(Path())):
    ...
    def mkdir(self, mode: int = 0, parents: bool = False, exist_ok: bool = True) -> None:
    ...
    def write_text(self, data: str, encoding: str | None = None, errors: str | None = None, newline: str | None = None):
...
def bitwarden_unlock(password_file: Path | None = None):
    ...
    if password_file.exists():
        ...
        if password_file.stat().st_mode & 0o777 != 0o600:
            logger.warning(f"Password file {password_file} has insecure permissions. Please set it to 600")
    else:
...
class Util:
    ...
    class Config:
        ...
        def readable_files(self):
            return [file for file, state in self.files if state in [File.readable, File.writable]]
                ...
                if field not in conf:
                    ...
                    if (env_var := self.parent.get_env_var(field)) is None:
                        ...
                        if field_info.required:
                            raise Exception(f"Configuration option `{field}` is required and unset")
                        else:
                            continue
                    conf[field] = env_var
            return conf
...
class BaseSettings(PydanticBaseSettings, metaclass=BaseSettingsMeta):
    ...
    def __init_subclass__(cls, **kwargs):
        ...
        if not app_name:
            raise TypeError("Subclasses of BaseSettings must include a Config class with an app_name attribute")
                ...
                for name in [f"uoft-{app_name}", f"shared/uoft-{app_name}"]:
                    ...
                    try:
                        ...
                        if not text:
                            ...
                            continue
                        logger.debug(
                            f"Successfully decrypted and loaded content from {path}, attempting to parse as TOML data"
                        )
