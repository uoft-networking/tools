...
def get_ap_groups():  # Create the 'get-ap-groups' command within typer.
    ...
    with s.mm_api_connection as host:
        ...
        console().print("Below you will find a list of valid AP_GROUPs to use in your input:")
...
def provision(
    ...
    aps_list = [(ap_name, ap_group, mac_address) for mac_address, ap_group, ap_name, *_ in reader]
    ...
    if failed_aps:
        ...
        for ap_error in failed_aps:
            console().print(f"{ap_error.ap.mac_address},{ap_error.ap.group},{ap_error.ap.name},{ap_error.args[0]}")


@run.command(  # Create the 'from-file' subcommand within 'provision'.
    short_help="De-provisions (deletes) a CSV list of MAC_ADDRESS,AP_GROUP,AP_NAME's, one per line.",
)
def deprovision(
    filename: Path = typer.Argument(
        ...,
    ...
    """
    De-provision a list of APs using information from a provided CSV file.
    Each row in the CSV file represents an AP to be removed.
    The CSV file must consist of 3 columns: MAC_ADDRESS, AP_GROUP, and AP_NAME.
    The CSV file must not contain a header row.\n
    ...
    reader = csv.reader(file, csv_format)
    aps_list = [(ap_name, ap_group, mac_address) for mac_address, ap_group, ap_name, *_ in reader]
    results = batch.Provisioner(on_error.value, dry_run).deprovision_aps(aps_list)
    failed_aps = [ap for ap in results if isinstance(ap, Exception)]

    if failed_aps:
        console().print("The following APs failed to deprovision:\nMAC_ADDRESS,AP_GROUP,AP_NAME,REASON")
        for ap_error in failed_aps:
            console().print(f"{ap_error.ap.mac_address},{ap_error.ap.group},{ap_error.ap.name},{ap_error.args[0]}")
