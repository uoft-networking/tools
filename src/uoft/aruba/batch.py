...
class AP(BaseModel):
    ...
    def validate_mac_address(cls, v):
        mac_address_validate_pattern = re.compile(r"^(?:[0-9A-Fa-f]{2}[:\-\.]?){5}(?:[0-9A-Fa-f]{2})$")
...
class Provisioner:
        ...
        for ap in inputs:
            try:
                ...
                if self.error_policy == "skip":
                    console().print(f"[red]ERROR[/] {e.args[0]} [red]SKIPPING[/] {e.ap}")
                    yield e
                else:
                    raise e
            except AlreadyExists as e:
                if "already correctly provisioned" in e.args[0]:
                    console().print(f"AP_NAME {e.ap.name} already correctly provisioned. [green]SKIPPING[/]")
                    yield e.ap
    ...
    def delete_existing_ap(self, old_ap):
        ...
        if self.dry_run:
            ...
            self.mobility_master.ap_provisioning.wdb_cpsec_delete_mac(old_ap["MAC-Address"])
            try:
    ...
    def provision_ap(self, name, group, mac_address):
        console().print(f"Provisioning {name} with MAC {mac_address} in group {group}...")
        ...
        if self.dry_run:
            ...
            self.mobility_master.ap_provisioning.wdb_cpsec_add_mac(ap.mac_address, ap.group, ap.name)
        console().print(msg)
        ...
        if self.dry_run:
            ...
            self.mobility_master.ap_provisioning.wdb_cpsec_modify_mac_factory_approved(ap.mac_address)
        console().print(msg)
    ...
    def _validate_group(self, ap: AP):
        ...
        console().print(f"Verifying AP_GROUP {ap.group} exists on controller...[green]GOOD[/]")
    ...
    def _validate_name(self, ap: AP):
        if ap.name in self.existing_aps_by_name:
            ...
            if existing_ap["MAC-Address"] == ap.mac_address:
                ...
                raise AlreadyExists(msg, ap=ap, old_ap=existing_ap)
        console().print(f"Verifying AP_NAME {ap.name} is not in use...[green]GOOD[/]")
    ...
    def _validate_mac(self, ap: AP):
        if ap.mac_address in self.existing_aps_by_mac:
            ...
            raise AlreadyExists(msg, ap=ap, old_ap=existing_ap)
        console().print(f"Verifying MAC {ap.mac_address} is not in use...[green]GOOD[/]")

    @cached_property
    def all_groups_by_name(self) -> set[str]:
        return {g["profile-name"].rpartition("'")[2] for g in self.controller.wlan.get_ap_groups()}
    ...
    def existing_aps_by_name(self):
        return {ap["AP-Name"]: ap for ap in self.existing_aps_in_allowlist if ap["AP-Name"]}

    @cached_property
    def existing_aps_by_mac(self):
        return {ap["MAC-Address"]: ap for ap in self.existing_aps_in_allowlist if ap["MAC-Address"]}
