...
class TestYAML:
    ...
    @pytest.mark.skipif(
        ...
        reason='utsc.core.yaml not available',
    )
    ...
    def test_CommentedSet(self):
        from utsc.core.yaml.constructor import CommentedSet
    ...
    def test_load_all_perserve_quotes(self):
        ...
        for x in yaml.load_all(s):
            data.append(x)
        buf = utsc.core.yaml.compat.StringIO()
        yaml.dump_all(data, buf)
        out = buf.getvalue()
        print(type(data[0]['a']), data[0]['a'])
        # out = utsc.core.yaml.round_trip_dump_all(data)
        print(out)
        assert out == s
