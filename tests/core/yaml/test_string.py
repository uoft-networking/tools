...
# from utsc.core.yaml.compat import ordereddict
...
class TestReplace:
    ...
    def test_replace_preserved_scalar_string(self):
        ...
        assert isinstance(so, utsc.core.yaml.scalarstring.LiteralScalarString)
        assert so == dedent("""
        bar
        bar
        bar
    ...
    def test_replace_double_quoted_scalar_string(self):
        ...
        data = round_trip_load(s, preserve_quotes=True)
        so = data['foo'].replace('foo', 'bar', 2)
        assert isinstance(so, utsc.core.yaml.scalarstring.DoubleQuotedScalarString)
        assert so == 'bar bar bar foo'


class TestWalkTree:
    def test_basic(self):
        from utsc.core.yaml.comments import CommentedMap
        from utsc.core.yaml.scalarstring import walk_tree
    ...
    def test_map(self):
        from utsc.core.yaml.compat import ordereddict
        from utsc.core.yaml.comments import CommentedMap
        from utsc.core.yaml.scalarstring import walk_tree, preserve_literal
        from utsc.core.yaml.scalarstring import DoubleQuotedScalarString as dq
        from utsc.core.yaml.scalarstring import SingleQuotedScalarString as sq
