...
Template File path [{}]> """.format(
        ...
        if not self.input_file:
            if not sys.stdout.isatty():
                sys.stderr.write(
                    "When piping this application's output, "
...
Item Type [assets]> """
        if not self.type:
            ...
            if not choice:  # User pressed enter to select default file path
                ...
                self.type = choice
        try:
            assert self.type in ["assets", "accessories", "consumables", "components"]
...
def main():
    ...
    def get_program_arguments():
        ...
        if "-h" in sys.argv or "--help" in sys.argv:
            ...
            parser.add_argument("-s", "--show-available-fields", action="store_true")
...
def unpack_template(input_path, tempdir) -> dict:
    ...
    if not input_path.exists():
        prompt = (
            "Error: " + str(input_path) + " does not seem to exist. "
...
def get_info_from_template(tempdir) -> dict:
    ...
    if not len(images_in_template) == 1:
        print(
            "Error: The template file you specified appears to have either "
