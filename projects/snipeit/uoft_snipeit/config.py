...
def get(name, values):
    ...
    for item in values:
        key_name = item["value"]
        result[key_name] = settings.get(item["value"], default=None)  # type: ignore
        if not result[key_name]:
            sensitive = item["sensitive"]  # bool
            result[key_name] = get_user_input(item["prompt"], sensitive)
            if item["optional"]:
                choice = get_user_input("Would you like to save this for later " "use? \n(yes/no)> ", sensitive=False)
...
def reset():
    os.unlink(SETTINGS_FILE_ROOT)  # type: ignore
