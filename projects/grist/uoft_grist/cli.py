        ...
        for group, aps in ap_groups.items():
            ...
            for client in unique_data:
                ...
                for range_name, ip_prefix in s.filter_ranges.items():
                    ...
                    if client_ip.startswith(ip_prefix) and group in inverteds[client_ap_name]:
...
def add_records_to_table(
    ...
    for group_name in department.apgroups:
        ...
        watermark_columns = []
        record: dict[str, str | int] = {"Date": now}
        ...
        if "Departmental_Users" in tally[group_name] and len(department.departmental_users) != 0:
            record["Departmental_Users"] = str(tally[group_name]["Departmental_Users"])
            watermark_columns.append(dict(id="Departmental_Users", fields={"type": "Int"}))
...
def clean():
    ...
    last_month = first - datetime.timedelta(days=1)  ### pick one
    days_30_ago = today - datetime.timedelta(days=30)  ### pick one
    for department in s.departments.values():
        grist = GristDocAPI(
            department.docid,
            server=s.grist_server,
        ...
        for group_name in department.apgroups:
            ...
            for table in tables:
                ...
                for value in values:
                    ...
                    if date < days_30_ago:  ### or 'last_month', pick one
                        value_int = []
