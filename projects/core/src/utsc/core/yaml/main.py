...
from .resolver import VersionedResolver, Resolver, BaseResolver  # NOQA
...
from .parser import Parser, RoundTripParser, RoundTripParserSC
from .composer import Composer
from .scanner import Scanner, RoundTripScanner, RoundTripScannerSC
...
if TYPE_CHECKING:  # MYPY
    from typing import Union, Any, Optional, Text  # NOQA
...
class YAML:
    def __init__(
        ...
        if "rt" in self.typ:
            ...
            self.Emitter = Emitter if pure or CEmitter is None else CEmitter
    ...
    def get_serializer_representer_emitter(self, stream, tlca):
        ...
        rslvr = BaseResolver if "base" in self.typ else Resolver
...
def add_implicit_resolver(
    ...
    if Loader:
        ...
        elif issubclass(Loader, (BaseLoader, SafeLoader, Loader, RoundTripLoader)):
            Resolver.add_implicit_resolver(tag, regexp, first)
        else:
            raise NotImplementedError
    if Dumper:
        ...
        elif issubclass(Dumper, (BaseDumper, SafeDumper, Dumper, RoundTripDumper)):
            Resolver.add_implicit_resolver(tag, regexp, first)
        else:
            raise NotImplementedError
